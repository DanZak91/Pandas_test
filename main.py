...
#Есть удобный способ группировки сразу с присвоением имен колонкам:
grouped_df2 = df.groupby('Category').agg(
    category_sum=('Values', 'sum'),
    category_mean=('Values', 'mean'),
    category_count=('Values', 'count')
)
print('\nгруппировки сразу с присвоением имен колонкам')
print(grouped_df2)
...
# Группировка по индексу и вычисление суммы значений в каждой группе/ level=0 в groupby() означает группировку по первому уровню индекса. Если индекс не многоуровневый, то это эквивалентно обычной группировке по индексу.
